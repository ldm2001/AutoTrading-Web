...
from service.stock_universe import INDICES, NAMES
...
class Market:
    ...
    async def candles_15m(self, code: str) -> list[dict]:
        ...
        candles = sorted(buckets.values(), key=lambda item: item["time"])
    ...
    async def target(self, code: str) -> float:
        ...
        resp.raise_for_status()
        out = resp.json()["output"]
        result = int(out[0]["stck_oprc"]) + (int(out[1]["stck_hgpr"]) - int(out[1]["stck_lwpr"])) * 0.5
